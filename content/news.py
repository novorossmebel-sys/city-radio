...
HOROSCOPE_TASK = (
    "Придумай короткое ироничное «гороскопное» напутствие на сегодня для читателей "
    "городского канала в Новороссийске — общее, без привязки к конкретному знаку зодиака. "
    "Тон — как мудрый голос за кадром, можно в манере Йоды (суть важнее строгой грамматики). "
    "Это не настоящая астрология, а лёгкий развлекательный формат — и это должно быть видно "
    "по тону, а не заявлено прямым текстом."
)


def post_horoscope() -> None:
    """Ежедневное «гороскоп-напутствие» (sources.yaml → goroskop, source_type: manual —
    контент выдумывается моделью, не агрегируется из источника, поэтому не участвует в
    опросе каналов content/digest_engine.py и нуждается в собственном триггере по расписанию,
    см. scheduler.py). До 2026-08-25 такого триггера не было вообще — generate_draft()
    существовал, но его никто не вызывал ни в старой схеме, ни в новой."""
    draft = generate_draft("Гороскоп", HOROSCOPE_TASK)
    from moderation import ModerationBot  # локальный импорт — как и в остальных run_*/post_* здесь
    bot = ModerationBot()
